...
function Aboutus() {
  ...
  <div
   ref={heroRef}
  className="text-white flex flex-col py-16 h-about-height items-center justify-center px-6 md:px-12 lg:px-24 text-center">
...
<div className='absolute top-48 lg:top-80 mt-10 lg:right-3 flex flex-col gap-4 ml-3 right-4'>
  ...
  <FontAwesomeIcon icon={faLinkedin} className='text-primary w-4 h-4 rounded-full'/>
    ...
    <p className="text-3xl font-bold  lg:mt-36 mb-4 leading-tight">About Us</p>
    ...
    <div className='flex md:flex-row mt-8 flex-col gap-3'>
    <Link to={"/"}>
    <button className='bg-primary rounded-full py-2 px-8 ml-3  mr-5'>Contact us</button>
    </Link>
    <Link to={"/"}>
    <button className= 'border-x-2 hover:text-primary rounded-full py-2 px-4 border-primary'>See our Partners <FontAwesomeIcon icon={faArrowRight}/></button>
    </Link>
    </div>
  </div>
  ...
  <div className="absolute hidden md:block top-40 lg:top-72 text-white text-sm  flex-col gap-4 ml-3 right-5">
      ...
      <div className="py-16 mt-28 mb-20 text-center">
        <h2 className="text-3xl font-bold ">Our Core <span className='text-primary'>Values</span></h2>
        <p className='mt-4 mb-10 text-xs text-gray-500 dark:text-gray-300'>Inform Africa excels in innovation with a dedicated team, making us the perfect partner for impactful projects.</p>
      ...
      <motion.div
  className="flex flex-col dark:bg-black  md:flex-row items-center md:items-start rounded-lg p-6 md:p-8 gap-8 md:gap-12"
  style={{
  ...
  initial="hidden"
  whileInView="visible"
  viewport={{ once: true, amount: 0.1 }} to trigger earlier
...
<p className="text-center text-sm ml-52 mr-52 text-gray-600 dark:text-gray-300 mb-10">
